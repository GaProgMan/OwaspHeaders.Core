...
- The `[Obsolete]` markers on the `UseExpectCt` and `ExpectCt` setters have been removed. They were originally added to warn callers who bypassed the (also obsolete) `UseExpectCt` builder extension; since those setters are no longer publicly accessible, the deprecation warning lives entirely on the builder extension.
...
- Added `SecureHeadersMiddlewareConfiguration.Validate()` which returns one descriptive issue string per `UseX` flag that is `true` without its matching configuration object populated. An empty list indicates a consistent configuration.
- `SecureHeadersMiddleware.InvokeAsync` now calls the validator on its first invocation. If any issues are reported, the consolidated message is logged at `LogLevel.Error` against event ID `3001` (`ConfigurationError`) and an `ArgumentException` is thrown.
- Added regression tests covering the scenario from issue #220, the multi-mismatch case (every offending flag is named in a single message), and the valid-configuration baseline.
- Added a logging test asserting that the consolidated configuration error fires the expected event ID and contains every offending flag name.

**Impact:**

- Behaviour change: an enabled `UseX` flag with a null configuration object now raises `ArgumentException` (with a message naming the offending flag and pointing at the builder) rather than `NullReferenceException`. The failure mode moves from a generic crash inside the framework to a diagnostic exception identifying the misconfiguration.
- Valid configurations built via `SecureHeadersMiddlewareBuilder` are unaffected; the validator runs once per middleware instance and short-circuits on subsequent requests.
- A future major version will additionally restrict the `UseX` flag and configuration-object setters so this misuse is prevented at compile time.
