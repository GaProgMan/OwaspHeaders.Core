...
Version 11 brings four breaking changes together: the supported runtimes shift to .NET 10 and .NET 11, the long-deprecated Expect-CT header is removed entirely, direct assignment to `SecureHeadersMiddlewareConfiguration` is no longer possible, and configuration moves to a delegate passed to `UseSecureHeadersMiddleware`. Configuration validation also moves from the first request to application startup. Each is detailed below.
